...
def multiples_of_ten(start, stop):
  ...
  if stop > 9:
    if start > 10:
      x = torch.arange(start= 10*(round(start//10)+1), end = stop, step=10,dtype= torch.float64) 
    else:
...
def normalize_columns(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  y = x - torch.mean(x, dim=0) 
  y = y/ torch.std(x, dim=0)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
