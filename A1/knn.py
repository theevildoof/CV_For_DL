...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(num_test):
    test_flatten = x_test[i].view(-1,1).clone()
    for j in range(num_train):
      train_flatten = x_train[j].view(-1,1).clone()
      dist = 0
      t = train_flatten - test_flatten 
      dist = t.pow(2).sum()
      dists[j,i] = dist
...
def compute_distances_one_loop(x_train, x_test):
  ...
  # Replace "pass" statement with your code
  for i in range(num_test):
    t = (x_train - x_test[i]).reshape(num_train,-1)
    t =  t.pow(2).sum(dim=1) 
...
def compute_distances_no_loops(x_train, x_test):
  ...
  dists = -2 * (x_train_flattened @ x_test_flattened.t()) + torch.sum(x_test_flattened**2, dim=1) + torch.sum(x_train_flattened**2, dim=1).reshape(-1, 1)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
